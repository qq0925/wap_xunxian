...
class Date
{
  ...
  */
  function convertLunarToSolar($year,$month,$date)
  {
    ...
    $res = date('Y-m-d-h-i-s', $res+$between*24*60*60);
    $day = explode('-', $res);
    $year = $day[0];
    $month= $day[1];
    $day = $day[2];
  ...
  */
  ...
  public function index()
  {
    ...
    $s = date('w');
    echo "[报时]<br/>时间：".date('Y')."-".date('m')."-".date('d')."  ".date('H').":".date('i').":".date('s')."  "."星期{$array[$s]}"."<br/>";
    echo "农历：".$today[0]."年  ".$today[1].$today[2]."<br/>";
    echo "干支：".$today[3]."年"."<br/>";
    echo "节气：".$month['name2']; 
    echo "<br/>生肖年：".$today[6];
  }
...
$obj = new Date(); // 实例化你的类，例如上面的示例代码中的类名为 YourClass
$today = $obj->index(); // 调用 index() 方法

...
$just_page = $encode->encode("cmd=gm_scene_new&sid=$sid");
events_steps_change($target_event,$sid,$dblj,$just_page,$steps_page);

function events_steps_change($target_event,$sid,$dblj,$just_page,$steps_page,$uid=null,$mid=null,$para=null){
                //事件逻辑
                    
                    global $encode;
                    global $steps_page;
                    global $just_page;
                    $event_data = npc_event_data_get($target_event,$dblj);
...
HTML;
                    echo $page_url;
                            ...
                            if($steps_page <$steps_count){
                                $steps_page +=1;
                                $page_continue = $encode->encode("cmd=npc_op_event&target_event=$target_event&just_page=$just_page&steps_page=$steps_page&sid=$sid");
                                $page_continue =<<<HTML

...
if(!$para){
task_event_change($target_event,$sid,$dblj,$just_page,$steps_page,$cmid,$parents_page);
}else {
    $input_value = $_POST['value'];
    $sql = "insert into system_player_inputs(sid,event_id,id,value)values('$sid','$target_event','$id','$input_value')";
    $dblj->exec($sql);
task_event_change($target_event,$sid,$dblj,$just_page,$steps_page,$cmid,$parents_page,null,null,$para);
}
function task_event_change($target_event,$sid,$dblj,$just_page,$steps_page,&$cmid,$parents_page,$uid=null,$mid=null,$para=null){
                //事件逻辑
                    global $encode;
                    global $steps_page;
                    global $parents_cmd;
                    ...
                    if($steps_page==0){
                    $register_triggle = checkTriggerCondition($event_cond,$dblj,$sid);//触发条件

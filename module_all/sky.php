...
function generateAreaLinks($dblj, $currentBelong, $encode, $cmid, $mid, $sid) {
    // 获取所有区域
    $sql = "SELECT `id`,`name` FROM `system_region` WHERE `sky_hide` = 0 and `id` != 0 ORDER BY `pos` ASC";
...
$sky_area_html = generateAreaLinks($dblj, $map_area_belong, $encode, $cmid, $mid, $sid);
}
...
$sky_city .=<<<HTML
<a href="?cmd=$skyto">{$sky_city_name}（{$distance}海里）</a><br/>
HTML;
}
$cmid = $cmid + 1;
$cdid[] = $cmid;

...
$query = "UPDATE system_event_evs_self SET items = REPLACE(items, :old_value, :new_value) WHERE belong = :belong AND id = :id";
// 准备并执行预处理语句
...
if (empty($result['items'])) {
    ...
    $query = "UPDATE system_event_evs_self SET items = :new_value where belong = '$event_id' and id = '$step_id'";
    $stmt = $dblj->prepare($query);
    $stmt->bindValue(':new_value', $string_new);
    $stmt->execute();
} else {
    // items 字段不为空，在原有值后面加上逗号和 $string_new
    $query = "UPDATE system_event_evs_self SET items = CONCAT(items, ',', :new_value) where belong = '$event_id' and id = '$step_id'";
    $stmt = $dblj->prepare($query);
    $stmt->bindValue(':new_value', $string_new);
    $stmt->execute();
}
...
$query = "UPDATE system_event_evs_self SET items = :newstring WHERE belong = '$event_id' and id = '$step_id'";
...
// 输出 HTML 锚点链接
$index_item_add = $encode->encode("cmd=game_event_itemadd_self&event_id=$event_id&step_id=$step_id&sid=$sid");
if($rows){
...
$item_change = $encode->encode("cmd=game_event_itemchange_self&item_id=$item_id&item_name=$item_name&item_count=$item_post_count&event_id=$event_id&step_id=$step_id&sid=$sid");
$item_remove = $encode->encode("cmd=game_event_itemchange_self&change=1&remove_id=$item_id&remove_count=$item_post_count&event_id=$event_id&step_id=$step_id&sid=$sid");
$item_list .=<<<HTML

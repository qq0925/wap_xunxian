...
$gm_game_globaleventdefine_skill_last = $encode->encode("cmd=game_event_destsadd&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
for ($i=0;$i<count($gm_ret);$i++){
    $hangshu +=1;
    $qy_name = $gm_ret[$i]['name'];
    $qy_id = $gm_ret[$i]['id'];
        $target_mid = $encode->encode("cmd=game_event_destsadd&step_belong_id=$step_belong_id&step_id=$step_id&scene_post_canshu=2&qy_id=$qy_id&sid=$sid");
        $map .=<<<HTML
...
echo $allmap;
    ...
    $gm_game_globaleventdefine_skill_last = $encode->encode("cmd=game_event_destsadd&scene_post_canshu=1&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
    $hangshu = 0;
    ...
    $mid = $cxallmap[$i]['mid'];
    $br++;;
    $target_mid = $encode->encode("cmd=game_event_destsadd&scene_post_canshu=add&step_belong_id=$step_belong_id&step_id=$step_id&mid=$mid&sid=$sid");
    $map .=<<<HTML
...
$game_event_destsadd = $encode->encode("cmd=game_event_destsadd&midexp=1&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
$last_list = $encode->encode("cmd=game_event_destsadd&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
$allmap = <<<HTML
<p>请输入目标场景id表达式<br/>
</p>
...
<a href="?cmd=$last_list">返回列表</a><br/>
HTML;
echo $allmap;
}
?>
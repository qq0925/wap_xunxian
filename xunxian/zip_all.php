...
function compressToZip($excludeFiles = [], $excludeDirs = []) {
    ...
    $zipFile = "xunxian_all.zip"; // 压缩后的文件名
    $zip = new ZipArchive();
    ...
    if ($zip->open($zipFile, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
        ...
        // 关闭并创建压缩包
        $zip->close();
        $gm_zip_update = $encode->encode("cmd=gm_game_zipfile&type=all&download=1&sid=$sid");
...
if (isset($download) && $download == 1) {
    ...
    $excludeFiles = ['xunxian_all.zip']; // 排除的文件
    $excludeDirs = ['.git', '.well-known','ache']; // 排除的文件夹

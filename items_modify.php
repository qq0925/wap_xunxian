...
try {
    ...
    // 定义需要处理的表和字段组合
    $operations = [
        ['system_event_evs_self', 'items'],
        ['system_event_evs', 'items'],
    ];
    ...
    foreach ($operations as $op) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $jsonData = itemsconvertStringToJson($row[$field]);
...
// 增强版转换函数（处理转义字符）
function itemsconvertStringToJson($input) {
    $result = [];
    // 处理转义逗号和竖线符号（如 a|b\,c => "b,c"）

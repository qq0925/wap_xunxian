...
if($skill_add_id){
// 检查 a_skills 字段是否为空
$query = "SELECT a_skills FROM system_event_evs where belong = '$step_belong_id' and id = '$step_id'";
...
if (empty($result['a_skills'])) {
    ...
    $query = "UPDATE system_event_evs SET a_skills = :new_value where belong = '$step_belong_id' and id = '$step_id'";
    $stmt = $dblj->prepare($query);
    ...
    $query = "UPDATE system_event_evs SET a_skills = CONCAT(a_skills, ',', :new_value) where belong = '$step_belong_id' and id = '$step_id'";
    $stmt = $dblj->prepare($query);
    $stmt->bindValue(':new_value', $skill_add_id);
    $stmt->execute();
}
...
$query = "UPDATE system_event_evs SET a_skills = :newstring WHERE belong = '$step_belong_id' and id = '$step_id'";
...
// 查询 system_event_evs 表获取 a_skills 字段的值
$query = "SELECT a_skills FROM system_event_evs where belong = '$step_belong_id' and id = '$step_id'";
...
// 输出 HTML 锚点链接
$index_skill_add = $encode->encode("cmd=game_event_skilladd&add=1&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
if($rows){
...
$skill_remove = $encode->encode("cmd=game_event_skilladd&change=1&remove_id=$skill_id&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
$skill_list .=<<<HTML

...
$sql = "SELECT items FROM system_event_evs WHERE belong = ? and id = ?"; 
...
$updateSql = "UPDATE system_event_evs SET items = ? WHERE belong = ? and id = ?";
...
// 输出 HTML 锚点链接
$index_item_add = $encode->encode("cmd=game_event_itemchange&add_action=1&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
...
foreach ($items as $key => $value) {
    ...
    $item_change = $encode->encode("cmd=game_event_itemchange&item_id=$key&item_name=$item_name&item_count=$value&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
    $item_remove = $encode->encode("cmd=game_event_itemchange&&item_id=$key&step_belong_id=$step_belong_id&step_id=$step_id&canshu=remove&sid=$sid");
    $item_list .=<<<HTML

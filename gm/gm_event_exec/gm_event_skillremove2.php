...
$gm_game_globaleventdefine_skill_last = $encode->encode("cmd=game_event_skillremove&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
if ($gm_cxjg){
...
for ($i=0;$i<count($gm_ret);$i++){
   ...
   // var_dump($gm_djname);
    ...
    $hangshu += 1;
    $skill_url = $encode->encode("cmd=game_event_skillremove&skill_remove_id=$skill_id&step_belong_id=$step_belong_id&step_id=$step_id&sid=$sid");
    $attr_html .=<<<HTML

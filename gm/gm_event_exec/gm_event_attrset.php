...
$sql = "SELECT s_attrs FROM system_event_evs WHERE belong = ? and id = ?"; 
...
$updateSql = "UPDATE system_event_evs SET s_attrs = ? WHERE belong = ? and id = ?";
...
foreach ($attrs as $key => $value) {
    ...
    $index_attr = $encode->encode("cmd=game_event_attrset_2&step_belong_id=$step_belong_id&step_id=$step_id&attr_key=$key&attr_value=$value&canshu=update&sid=$sid");
    $attr_html .=<<<HTML
...
$index_attr_add = $encode->encode("cmd=game_event_attrset_2&step_belong_id=$step_belong_id&step_id=$step_id&canshu=add&sid=$sid");
$data_in = $encode->encode("cmd=game_event_attrset_2&step_belong_id=$step_belong_id&step_id=$step_id&canshu=in&sid=$sid");
$gm_html =<<<HTML

...
$dblj->exec($sql);
$sql = "update `system_item_op` set link_event = 0 where id = '$op_id'";
$dblj->exec($sql);
}
...
$event_text =<<<HTML
<a href="?cmd=$item_op_events">定义事件</a>
HTML;
}else{
$item_op_events = $encode->encode("cmd=game_main_event&gm_post_canshu=item_op&main_id=$op_id&event_id=$op_link_event&sid=$sid");
$item_op_events_del = $encode->encode("cmd=game_main_event&gm_post_canshu=item_op&del_event=1&main_id=$op_id&event_id=$op_link_event&sid=$sid");
$event_text =<<<HTML
<a href="?cmd=$item_op_events">修改事件</a><a href="?cmd=$item_op_events_del">删除事件</a>
HTML;
}

...
以下！！！代表未施工
1、基本信息
这里定义游戏的各种最基本的信息，如：游戏名称、游戏简介、游戏状态、宠物出战和组队人数上限、以及人物帮派技能升级公式和升级条件、默认技能和入口场景等。

2、定义属性
这里可以定义人物、NPC、物品、场景、技能等的公共属性。
例如，如果给NPC属性定义了【年龄】属性，那么，在设计具体的电脑
人物基本属性时，就有【年龄】一项可供定义。如果把“年龄”属性定义为可显示的，那么玩家在使用默认模板查看NPC页面时可以看到NPC的【年龄】属性。
举例说明：
1.属性标识就是属性的ID，你可以设置为任何英文名，如age
2.属性名称就是属性的名称，如【年龄】
3.值类型，数值类为int型数字，字符串为varchar型字符串，逻辑值为tinyint型默认0为false、1为true，一般选择【数值类】
4.初始值：就是一个初始的数字，如0
5.是否显示：就是这个属性会不会显示在页面上
...
7、设计地图
可以设计地图的区域，用以区分地图板块，分区下可设计具体场景。

8、设计物品
分为消耗品、兵器、防具、书籍、任务物品、其它，其中兵器和防具还有子类别。
...
{u.ic.物品id}：用户身上某个物品的数量，如u.ic.i1。
{u.jv.技能id}：用户的某项技能的等级，如u.jv.j1。
...
所有的注册事件，登录事件，只有第一个步骤有效。
在物品事件中，触发条件失败不生成返回链接。
INPUT寿命:一个执行步骤
...
刷新场景npc（主要针对怪物）：会找到当前场景怪物的源id，而后进行刷新。格式：怪物源id1，怪物源id2
刷新场景物品：如字面意思
...
35、增加ut|ot.fight_umsg和ut|ot.fight_omsg设置和获得当前一招产生的描述，umsg为攻击的信息，omsg为受攻击的信息

...
$gm_game_globaleventdefine_skill_last = $encode->encode("cmd=gm_type_npc&gm_post_canshu=5&npc_id=$npc_id&sid=$sid");
if ($gm_cxjg){
...
for ($i=0;$i<count($gm_ret);$i++){
   ...
   // var_dump($gm_djname);
    ...
    $hangshu += 1;
    $skill_url = $encode->encode("cmd=gm_type_npc&gm_post_canshu=5&skill_name=$skill_name&skill_add_id=$skill_id&add=2&npc_id=$npc_id&sid=$sid");
    $attr_html .=<<<HTML

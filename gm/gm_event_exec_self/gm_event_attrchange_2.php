...
$gm_game_selfeventdefine_attr_last = $encode->encode("cmd=game_event_attrchange_self&event_id=$event_id&step_id=$step_id&sid=$sid");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['excel_file'])) {
    ...
    // 首先获取当前的m_attrs值
    $query = "SELECT m_attrs FROM system_event_evs_self WHERE belong = :evs_belong AND id = :evs_id";
    ...
    // 更新数据库
    $update_sql = "UPDATE system_event_evs_self SET m_attrs = :m_attrs WHERE belong = :evs_belong AND id = :evs_id";
...
$gm_game_selfeventdefine_attr_last = $encode->encode("cmd=game_event_attrchange_self&event_id=$event_id&step_id=$step_id&sid=$sid");
$attrchange_post = $encode->encode("cmd=game_event_attrchange_self&event_id=$event_id&step_id=$step_id&sid=$sid");

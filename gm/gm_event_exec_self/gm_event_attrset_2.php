...
$gm_game_selfeventdefine_attr_last = $encode->encode("cmd=game_event_attrset_self&event_id=$event_id&step_id=$step_id&sid=$sid");
...
// 生成并保存Excel文件
$excelFileName = 'in_data/attr_setting_self.xlsx';
$writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
$writer->save($excelFileName);

// 生成下载链接
$downloadLink = '<a href="' . $excelFileName . '" download>下载Excel导入模板文件</a><br/>';
$last = "<a href='?cmd=$gm_game_selfeventdefine_attr_last'>返回上级</a><br/>";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['excel_file'])) {
    ...
    // 首先获取当前的s_attrs值
    $query = "SELECT s_attrs FROM system_event_evs_self WHERE belong = :evs_belong AND id = :evs_id";
    $stmt = $dblj->prepare($query);
    $stmt->bindParam(':evs_belong', $event_id);
    ...
    $attrs_array = [];
    if (!empty($current_data['s_attrs'])) {
        // 尝试解析JSON
        $decoded = json_decode($current_data['s_attrs'], true);
        if (is_array($decoded)) {
            $attrs_array = $decoded;
        }
    }
    ...
    // 将处理后的数组转换为JSON
    $new_s_attrs = json_encode($attrs_array, JSON_UNESCAPED_UNICODE);
    
    // 更新数据库
    $update_sql = "UPDATE system_event_evs_self SET s_attrs = :s_attrs WHERE belong = :evs_belong AND id = :evs_id";
    $stmt = $dblj->prepare($update_sql);
    $stmt->bindParam(':s_attrs', $new_s_attrs);
    $stmt->bindParam(':evs_belong', $event_id);
    $stmt->bindParam(':evs_id', $step_id);
    ...
    try {
        ...
        echo "<div>更新后的属性数据：<pre>" . htmlspecialchars($new_s_attrs) . "</pre></div>";
...
if (!empty($current_data['s_attrs'])) {
    $current_attrs_html .= '<pre>' . htmlspecialchars($current_data['s_attrs']) . '</pre>';
} else {

...
if($skill_remove_id){
// 检查 r_skills 字段是否为空
$query = "SELECT r_skills FROM system_event_evs_self where belong = '$event_id' and id = '$step_id'";
...
if (empty($result['r_skills'])) {
    ...
    $query = "UPDATE system_event_evs_self SET r_skills = :new_value where belong = '$event_id' and id = '$step_id'";
    $stmt = $dblj->prepare($query);
    ...
    $query = "UPDATE system_event_evs_self SET r_skills = CONCAT(r_skills, ',', :new_value) where belong = '$event_id' and id = '$step_id'";
    $stmt = $dblj->prepare($query);
    $stmt->bindValue(':new_value', $skill_remove_id);
    $stmt->execute();
}
...
$query = "UPDATE system_event_evs_self SET r_skills = :newstring WHERE belong = '$event_id' and id = '$step_id'";
...
// 查询 system_event_evs_self 表获取 r_skills 字段的值
$query = "SELECT r_skills FROM system_event_evs_self where belong = '$event_id' and id = '$step_id'";
...
// 输出 HTML 锚点链接
$index_skill_add = $encode->encode("cmd=game_event_skillremove_self&add=1&event_id=$event_id&step_id=$step_id&sid=$sid");
if($rows){
...
$skill_remove = $encode->encode("cmd=game_event_skillremove_self&change=1&remove_id=$skill_id&event_id=$event_id&step_id=$step_id&sid=$sid");
$skill_list .=<<<HTML

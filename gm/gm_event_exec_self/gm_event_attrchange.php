...
$sql = "SELECT m_attrs FROM system_event_evs_self WHERE belong = ? and id = ?"; 
...
$updateSql = "UPDATE system_event_evs_self SET m_attrs = ? WHERE belong = ? and id = ?";
$updateStmt = $dblj->prepare($updateSql);
$updateStmt->execute([$newJson,$event_id, $step_id]);
}
// 假设你已经连接到数据库并获取了数据

// 查询 system_event_evs_self 表获取 m_attrs 字段的值
...
foreach ($attrs as $key => $value) {
    ...
    $index_attr = $encode->encode("cmd=game_event_attrchange_2_self&event_id=$event_id&step_id=$step_id&attr_key=$key&attr_value=$value&canshu=update&sid=$sid");
    $attr_html .=<<<HTML
...
$index_attr_add = $encode->encode("cmd=game_event_attrchange_2_self&event_id=$event_id&step_id=$step_id&canshu=add&sid=$sid");
$data_in = $encode->encode("cmd=game_event_attrchange_2_self&event_id=$event_id&step_id=$step_id&canshu=in&sid=$sid");
$gm_html =<<<HTML
